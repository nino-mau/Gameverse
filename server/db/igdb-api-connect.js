...
const CLIENT_SECRET = 'lmtfy8z7biwe32mmx2qla0qdro2ktz';
...
// Function to search for games by name - IMPROVED APPROACH
async function searchGames(token, gameNames) {
   const results = [];

   for (const gameName of gameNames) {
      // First try: exact name search with quotes and escaping
      const escapedName = gameName.replace(/"/g, '\\"');

      // Create a more flexible search query
      const query = `
      search "${escapedName}";
      fields name, cover.*, artworks.*, screenshots.*;
      limit 5;
    `;
      ...
      try {
         ...
         if (gameData && gameData.length > 0) {
            ...
            const exactMatch = gameData.find(
               (g) => g.name.toLowerCase() === gameName.toLowerCase(),
            );

            const bestMatch = exactMatch || gameData[0];
            results.push({
               originalName: gameName,
               gameData: bestMatch,
            });
            console.log(`Found result for: ${gameName} → ${bestMatch.name}`);
         } else {
            // Second try: more flexible search approach
            const fallbackQuery = `
          fields name, cover.*, artworks.*, screenshots.*;
          limit 5;
          where name ~ "${escapedName.split(' ')[0]}";
        `;

            console.log(`No direct results, trying alternative search for: ${gameName}`);

            const fallbackResponse = await fetch('https://api.igdb.com/v4/games', {
               method: 'POST',
               headers: {
                  'Client-ID': CLIENT_ID,
                  Authorization: `Bearer ${token}`,
            ...
            if (fallbackResponse.ok) {
               ...
               if (fallbackData && fallbackData.length > 0) {
                  ...
                  if (bestSimilarity > 0.5) {
                     ...
                     console.log(
                        `Found partial match for: ${gameName} → ${bestMatch.name} (similarity: ${bestSimilarity.toFixed(2)})`,
                     );
                  } else {
...
// Function to extract and format image URLs with fallback to screenshots
function extractImageUrls(gameResults) {
   return gameResults.map((result) => {
      const { originalName, gameData: game } = result;
      ...
      // Format cover URL (if available)
      if (game.cover && game.cover.url) {
         // Convert from thumbnail to full size by replacing 't_thumb' with 't_cover_big'
         formattedGame.cover = `https:${game.cover.url.replace('t_thumb', 't_cover_big')}`;
      }
...
// Main function to get images from a list of game names
async function getGameImages(gameNames) {
   try {
      // Validate input
      if (!Array.isArray(gameNames)) {
         throw new Error('gameNames must be an array of strings');
      }

      // Filter out any non-string entries
      const validGameNames = gameNames.filter((name) => typeof name === 'string');

      console.log(`Searching for ${validGameNames.length} games...`);
      ...
      // Search for games
      const gameResults = await searchGames(token, validGameNames);
      console.log(`Found ${gameResults.length} games out of ${validGameNames.length} requested`);

      // Extract and format image URLs
      const gamesWithImages = extractImageUrls(gameResults);
...
// Example usage with game names that had issues
const games = [
   {
      id: 125642,
...
const gameNames = games.map((obj) => obj.name);

// Execute the function
getGameImages(gameNames)
   .then((results) => {
      console.log(`Successfully processed ${results.length} games`);
      console.log(JSON.stringify(results, null, 2));

      // Save results to a file
      fs.writeFileSync('game-images.json', JSON.stringify(results, null, 2));
      console.log('Results saved to game-images.json');
   })
   .catch((error) => {
